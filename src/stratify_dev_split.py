...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Stratify Train/Dev split.")
    parser.add_argument("--train-raw", default=None, help="Path to raw train split JSONL")
    parser.add_argument("--dev-raw", default=None, help="Path to raw dev split JSONL")
    parser.add_argument("--test-gold", default=None, help="Path to gold test split JSONL")
    parser.add_argument("--outdir", default=None, help="Directory to save output splits")
    ...
    # Defaults
    train_raw_path = Path(args.train_raw) if args.train_raw else project_dir / "data" / "annotated" / "train.jsonl"
    dev_raw_path = Path(args.dev_raw) if args.dev_raw else project_dir / "data" / "annotated" / "validation.jsonl"
    test_gold_path = Path(args.test_gold) if args.test_gold else project_dir / "data" / "annotated" / "test.jsonl"
    out_dir = Path(args.outdir) if args.outdir else project_dir / "data" / "annotated"

    # Check if files exist, fallback if not found
    if not train_raw_path.exists():
        train_raw_path = src_dir / "train_raw.jsonl"
    if not dev_raw_path.exists():
        dev_raw_path = src_dir / "dev_raw.jsonl"
    if not test_gold_path.exists():
        test_gold_path = src_dir / "test_gold_300.jsonl"

    if not train_raw_path.exists() or not dev_raw_path.exists():
        print(f"Error: Required training files not found at:")
        print(f"  Train Raw: {train_raw_path}")
        print(f"  Dev Raw:   {dev_raw_path}")
        print(f"Please ensure they exist or specify their paths via command line arguments.")
        sys.exit(1)
    ...
    # 1. Load the original raw Silver dataset to ensure clean re-stratification
    silver_records = []
    ...
    # 2. Extract stratified 300 samples for dev, leaving remaining for train
    ...
    if test_gold_path.exists():
        with open(test_gold_path, encoding="utf-8") as f:
            for line in f:
                test_records.append(json.loads(line))
    else:
        print(f"Warning: Test gold file not found at {test_gold_path}, output will not include combined test set.")
    ...
    save_jsonl(dev_records, out_dir / "validation.jsonl")
    if test_records:
        save_jsonl(test_records, out_dir / "test.jsonl")

    # Combine into full_dataset
    full_dataset = train_records + dev_records + test_records
    save_jsonl(full_dataset, out_dir / "full_dataset.jsonl")

    # Save splits for the NER training workflow
    save_jsonl(train_records, out_dir / "train_full.jsonl")
    save_jsonl(dev_records, out_dir / "dev_full.jsonl")
